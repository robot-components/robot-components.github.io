...
const EMPTY_FORM = { title: "", cat: "공지", body: "", pinned: false };
...
export default function NoticePage({ adminUser }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    if (editForm.id) {
      await supabase.from("notices").update({ title: editForm.title, cat: editForm.cat, body: editForm.body, pinned: editForm.pinned }).eq("id", editForm.id);
    } else {
      const maxIdx = notices.length > 0 ? Math.max(...notices.map(n => n.order_idx ?? 0)) : -1;
      const today = new Date().toISOString().slice(0, 10);
      await supabase.from("notices").insert({ title: editForm.title, cat: editForm.cat, body: editForm.body, pinned: editForm.pinned, order_idx: maxIdx + 1, date: today });
    }
    setSaving(false);
    setEditForm(null);
    fetchNotices();
          ...
          {detail.files?.length > 0 && (
            <div style={{ marginTop: 24, borderTop: "1px solid #f1f5f9", paddingTop: 16 }}>
              ...
              {detail.files.map((f, i) => (
                <a key={i} href={f.url} download style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "#eff6ff", border: "1px solid #bfdbfe", color: "#1d4ed8", padding: "7px 14px", borderRadius: 8, textDecoration: "none", fontSize: 13, fontWeight: 500, marginRight: 8 }}>
                  ↓ {f.name}
                </a>
              ))}
            </div>
            ...
            <div key={n.id} style={{ background: n.pinned ? "#f0f9ff" : "#fff", borderBottom: idx < filtered.length - 1 ? "1px solid #f1f5f9" : "none", padding: "13px 18px", display: "flex", alignItems: "center", gap: 10 }}>
              ...
              {adminUser && (
                <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                  ...
                  <button onClick={() => setEditForm({ id: n.id, title: n.title, cat: n.cat, body: n.body || "", pinned: n.pinned || false })} title="편집"
