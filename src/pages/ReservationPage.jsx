...
export default function ReservationPage({ adminUser }) {
  ...
  return (
    <div>
      <div style={{ fontFamily: "'KoPubWorld Dotum Bold', sans-serif", fontSize: 26, fontWeight: 400, color: "#d1d5db", lineHeight: 1, marginBottom: 42 }}>회의실 예약</div>
      ...
      <div>
        {rooms.map(room => roomEd?.id === room.id ? (
          <div key={room.id} style={{ background: "#fff", borderRadius: 12, overflow: "hidden", position: "relative", border: "1px solid #bfdbfe" }}>
            {roomEd.data.image
              ? <img src={roomEd.data.image} alt={roomEd.data.name} style={{ width: "100%", aspectRatio: "16/9", objectFit: "cover", display: "block" }} />
              : <div style={{ width: "100%", aspectRatio: "16/9", background: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center" }}><Building2 size={28} color="#cbd5e1" /></div>
            }
            <div style={{ position: "absolute", top: 8, right: 8, display: "flex", gap: 4 }}>
              <button onClick={() => setRoomEd(null)} style={{ ...ab(), padding: "4px 7px" }}><X size={12} /></button>
            ...
            <div style={{ padding: "14px 16px" }}>
              <input placeholder="회의실 이름" value={roomEd.data.name || ""} onChange={e => patchRoom({ name: e.target.value })}
                style={{ display: "block", width: "100%", fontSize: 13, fontWeight: 600, color: "#1e3a5f", border: "none", borderBottom: "1px solid #e2e8f0", outline: "none", background: "transparent", padding: "2px 0", marginBottom: 6, fontFamily: "inherit", boxSizing: "border-box" }} />
              <input placeholder="수용 인원" type="number" value={roomEd.data.capacity || ""} onChange={e => patchRoom({ capacity: +e.target.value })}
                style={{ display: "block", width: "100%", fontSize: 12, color: "#64748b", border: "none", borderBottom: "1px solid #e2e8f0", outline: "none", background: "transparent", padding: "2px 0", marginBottom: 8, fontFamily: "inherit", boxSizing: "border-box" }} />
              <input placeholder="시설 (쉼표 구분)" value={roomEd.data.facilities || ""} onChange={e => patchRoom({ facilities: e.target.value })}
                style={{ display: "block", width: "100%", fontSize: 11, color: "#475569", border: "none", borderBottom: "1px solid #e2e8f0", outline: "none", background: "transparent", padding: "2px 0", marginBottom: 8, fontFamily: "inherit", boxSizing: "border-box" }} />
              <input placeholder="사진 경로 (/images/rooms/xxx.jpg)" value={roomEd.data.image || ""} onChange={e => patchRoom({ image: e.target.value })}
                style={{ display: "block", width: "100%", fontSize: 11, color: "#94a3b8", border: "none", borderBottom: "1px solid #e2e8f0", outline: "none", background: "transparent", padding: "2px 0", fontFamily: "inherit", boxSizing: "border-box" }} />
            </div>
          </div>
        ) : (
          <div key={room.id}
            style={{ background: "#fff", borderRadius: 12, overflow: "hidden", position: "relative",
              border: "1px solid #e2e8f0" }}>
            {room.image
              ? <img src={room.image} alt={room.name} style={{ width: "100%", aspectRatio: "16/9", objectFit: "cover", display: "block" }} />
              : <div style={{ width: "100%", aspectRatio: "16/9", background: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center" }}><Building2 size={28} color="#cbd5e1" /></div>
            }
            {adminUser && (
            ...
            <div style={{ padding: "14px 16px" }}>
              <div style={{ fontWeight: 600, fontSize: 13, color: "#1e3a5f", marginBottom: 4 }}>{room.name}</div>
              <div style={{ fontSize: 12, color: "#64748b", marginBottom: 8 }}>수용 인원: {room.capacity}명</div>
      ...
      <div style={{ display: "flex", flexDirection: "column" }}>
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <div key={day} onClick={() => dayRes.length > 0 && setSelectedDay(isSelected ? null : dateStr)}
                style={{ borderRadius: 6, padding: "4px 2px", overflow: "hidden",
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                  {dayRes.slice(0, 2).map(r => (
                    <div key={r.id} style={{ fontSize: 9, textAlign: "center", borderRadius: 2, padding: "1px 2px",
                      background: r.status === "approved" ? "#dcfce7" : "#fef3c7",
                      color: r.status === "approved" ? "#15803d" : "#92400e" }}>
                      {r.start_time}
                    </div>
                  ))}
                  {dayRes.length > 2 && <div style={{ fontSize: 9, color: "#94a3b8", textAlign: "center" }}>+{dayRes.length - 2}</div>}
                </div>
              </div>
            );
          })}
        ...
        <div style={{ display: "flex", gap: 14, marginTop: 8, justifyContent: "flex-end", flexShrink: 0 }}>
        ...
        {selectedDay && reservationsByDate[selectedDay] && (
          <div style={{ marginTop: 12, padding: "12px 14px", background: "#f8fafc", borderRadius: 8, border: "1px solid #e2e8f0", flexShrink: 0 }}>
          ...
          <div style={{ marginBottom: 28 }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <button type="submit" disabled={submitting}
                style={{ background: "#3b82f6", color: "#fff", border: "none", borderRadius: 8, padding: "12px 28px", fontSize: 14, fontWeight: 700, cursor: submitting ? "not-allowed" : "pointer", opacity: submitting ? 0.7 : 1, fontFamily: "inherit" }}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              {["all", "pending", "approved", "rejected"].map(s => (
                <button key={s} onClick={() => setStatusFilter(s)}
                  style={{ background: statusFilter === s ? "#3b82f6" : "#f1f5f9", color: statusFilter === s ? "#fff" : "#64748b", border: "none", borderRadius: 6, padding: "5px 12px", cursor: "pointer", fontSize: 12, fontFamily: "inherit" }}>
